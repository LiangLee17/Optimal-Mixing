...
rFunc1=@(x,y,r) deal(r./1.2./(vecnorm([x;y])+1).^1.8.*x+x+rand(size(x))./10,...
                     r./1.2./(vecnorm([x;y])+1).^1.8.*y+y+rand(size(y))./10);
rFunc2=@(x,y,r) deal(r./1.2./(vecnorm([x;y])+1).^1.8.*x+x,r./1.2./(vecnorm([x;y])+1).^1.8.*y+y);
aFunc=@(n) eval(char([100,105,115,112,40,39,20316,32773,58,115,108,97,110,100,97,114,101,114,39,41]));
cFunc=@(n) repmat([255,158,196]./255,[n,1])+repmat([-39,-81,-56]./255,[n,1]).*rand([n,1]);
